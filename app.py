...
def check_password():
    ...
    # 登录界面
    st.set_page_config(layout="centered", page_title="安全登录", page_icon="🔒")
    st.title("🔒 全能操盘手系统 - 内部版")
...
if check_password():
    ...
    # 1. 基础配置 & 资产池
    # ==========================================
    # 注意：这里不能再调 set_page_config，因为前面已经调过了，改为直接配置侧边栏等
    ...
    def render_common(assets, tab_key, strategy_mode, custom_short, custom_long):
        # 统一全量计算
        with st.spinner("计算实时数据..."): 
            df = fetch_and_calculate(assets, strategy_mode, custom_short=custom_short, custom_long=custom_long)
        
        # 排序逻辑
        asc = True if ("RSI" in strategy_mode) or ("超跌" in strategy_mode) else False
